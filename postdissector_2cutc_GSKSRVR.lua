...
local proto_gsksrvr_post = Proto("gsksrvr_post","2cUTC info field from GSKSRVR")

-- we define a "2cutc.info" field

local info_field = ProtoField.string("2cutc.info", "2cUTC_Info")
proto_gsksrvr_post.fields = { info_field }
...
-- any fielde from GSKSRVR?

register_postdissector(proto_gsksrvr_post)

--POSTDISSECTOR
--POSTDISSECTOR
--POSTDISSECTOR

function proto_gsksrvr_post.dissector(tvb, pinfo, tree)
    ...
    if component ~= "GSKSRVR" then return end
