...
local proto_systcpda_post = Proto("systcpda_post","2cUTC info field from SYSTCPDA")

-- we define a "2cutc.info" field

local info_field = ProtoField.string("2cutc.info", "2cUTC_Info")
proto_systcpda_post.fields = { info_field }
...
-- any fields from SYSTCPDA?
-- remember to check any one of these for "nil" in the post-dissector

register_postdissector(proto_systcpda_post)

--POSTDISSECTOR
--POSTDISSECTOR
--POSTDISSECTOR

function proto_systcpda_post.dissector(tvb, pinfo, tree)
    ...
    if component ~= "SYSTCPDA" then return end
