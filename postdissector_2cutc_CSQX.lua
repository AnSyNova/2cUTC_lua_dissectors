...
local proto_csqx_post = Proto("csqx_post","2cUTC info field from CSQX")

-- we define a "2cutc.info" field

local info_field = ProtoField.string("2cutc.info", "2cUTC_Info")
proto_csqx_post.fields = { info_field }
...
-- any fields from CSQX?
-- remember to check any one of these for "nil" in the post-dissector

register_postdissector(proto_csqx_post)

--POSTDISSECTOR
--POSTDISSECTOR
--POSTDISSECTOR

function proto_csqx_post.dissector(tvb, pinfo, tree)
    ...
    if component ~= "CSQX" then return end
